...
function FormMusteriContainer() {
  ...
  const btnSave = async () => {
    let flatTypeID = "";
    for (var i = 0; i < _flatTypeID.length; i++) {
      if (flatTypeID === "") {
        flatTypeID += _flatTypeID[i];
      } else {
        flatTypeID += "," + _flatTypeID[i];
      }
    }
    console.log("Son hâl:", flatTypeID);
    console.log("Olması gerektiği gibi mi?", _flatTypeID);
